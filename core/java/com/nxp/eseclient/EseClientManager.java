...
public final class EseClientManager{
    ...
    Class nfcStub, spiStub, nxpNfcStub;
    ...
    private Object getNxpNfcServiceInterface() {
        Object b = null, nxpNfcService = null;
        if(sNfcService == null)
        {
        ...
        try {
            ...
            for(int i=0; i<mthd.length; i++) {
                ...
                if(mthd[i].getName().compareTo("getNfcAdapterVendorInterface") == 0) {
                    getNxpAdapterMthd = mthd[i];
                }
            }
            b = getNxpAdapterMthd.invoke(sNfcService,"nxp");
        } catch (Exception e) {
            Log.e(TAG, "getNfcAdapterVendorInterface method not found", e);
            throw new UnsupportedOperationException();
        }
        try{
            nxpNfc = System.class.forName("com.nxp.nfc.INxpNfcAdapter");
            Class cls[] = nxpNfc.getClasses();
            for(int i=0; i<cls.length; i++) {
                Log.e(TAG, "cls["+i+"] = "+ cls[i].getSimpleName());
                if(cls[i].getSimpleName().compareTo("Stub") == 0) {
                    nxpNfcStub = cls[i];
        ...
        if(nxpNfcStub != null) {
            try{
                Method asInterfaceMethod = nxpNfcStub.getDeclaredMethod("asInterface", IBinder.class);
                nxpNfcService = asInterfaceMethod.invoke(null,b);
            } catch (Exception e) {
                Log.e(TAG, "nxpNfc-asInterface method not found", e);
                throw new UnsupportedOperationException();
            }
        }
        return nxpNfcService;
    }
